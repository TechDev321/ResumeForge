...
async def generate_cover_letter(
    ...
    resume_bytes = await _read_resume_upload(resume)
...
@app.post("/api/generate-answer", response_model=AnswerResponse)
async def generate_answer(
    ...
    cache_dir = None
    if os.getenv("QUESTION_CACHE", "0") == "1":
        cache_dir = Path("cache") / "question"
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Answer generation failed: {e}") from e
